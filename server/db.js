...
function seedSchemes() {
  const count = db.prepare('SELECT COUNT(*) as total FROM schemes').get();
  if (count.total >= 9) {
    return;
  }

  const insertStmt = db.prepare(`
    INSERT OR REPLACE INTO schemes (id, name, level, farmer_type, benefit_summary, eligibility_rules, official_link)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const seedAll = db.transaction(() => {
    for (const s of schemes) {
      insertStmt.run(s.id, s.name, s.level, s.farmer_type, s.benefit_summary, s.eligibility_rules, s.official_link);
    }
  });

  seedAll();
  console.log('Database seeded with 9 schemes successfully.');
}
